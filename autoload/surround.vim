...
function! surround#dosurround(...) abort " {{{1
  let scount = v:count1
  let char = (a:0 ? a:1 : surround#inputtarget())
  let spc = ""
  if char =~ '^\d\+'
...
function! surround#changesurround(...) abort " {{{1
  ...
  call surround#dosurround(a,b,a:0 && a:1)
endfunction " }}}1

function! surround#opfunc(type, ...) abort " {{{1
  if a:type ==# 'setup'
    let &opfunc = 'surround#opfunc'
    return 'g@'
  endif
  let char = s:inputreplacement()
  if char == ""
...
function! surround#opfunc2(...) abort
  if !a:0 || a:1 ==# 'setup'
    let &opfunc = 'surround#opfunc2'
    return 'g@'
  endif
  call surround#opfunc(a:1, 1)
endfunction " }}}1
